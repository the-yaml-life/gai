...
class OpenRouterAdapter:
    """OpenRouter API adapter - handles HTTP requests and response parsing"""

    def __init__(self, api_key: str, api_url: str = "https://openrouter.ai/api/v1/chat/completions", verbose: bool = False):
    ...
    def _log(self, message: str):
        if self.verbose:
            print(f"[OpenRouterAdapter] {message}")
    ...
    def request(self, messages: List[Dict[str, str]], model: str, **kwargs) -> Dict[str, Any]:
        ...
        headers = {
            ...
            "HTTP-Referer": "https://github.com/yourusername/gai",
            "X-Title": "gai - Git AI Assistant"
        }
        ...
        try:
            ...
            raise ProviderError(
                f"OpenRouter API timeout after 60s: {str(e)}",
                status_code=None,
                provider="openrouter"
            )
            ...
            raise ProviderError(
                f"OpenRouter API request error: {str(e)}",
                status_code=None,
                provider="openrouter"
            )
        ...
        if response.status_code == 200:
            ...
            raise ProviderError(
                f"OpenRouter API HTTP {response.status_code}: {error_details}",
